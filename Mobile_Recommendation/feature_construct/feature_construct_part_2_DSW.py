...
精简特征版本 - 每类只保留5个核心特征
Part 2: 2014-11-29 ~ 2014-12-04 → 预测 2014-12-05
...
print("开始构建Part 2精简特征集 (DSW全量数据)...") # 【修改】
print("=" * 60)
...
path_df_part_2 = "Mobile_Recommendation/data/mobile/df_part_2.csv"  # 【不修改】Part2数据来自divide_data_set.py的输出
path_df_part_2_uic_label = "Mobile_Recommendation/data/mobile/df_part_2_uic_label.csv"  # 【不修改】
...
# 检查输入文件 【不修改】
if not os.path.exists(path_df_part_2):
    print(f"错误: Part2文件不存在 {path_df_part_2}")
    exit(1)
else:
    file_size = os.path.getsize(path_df_part_2) / (1024 * 1024)  # MB
    print(f"Part2文件检查通过: {path_df_part_2} ({file_size:.1f}MB)")
...
###########################################
'''Step 2.1 用户(U)特征 - 精简版(5个特征) - DSW 48核心优化 【修改注释】
...
print(f"   分块处理Part2数据 (DSW大内存chunk_size: {chunk_size:,})...") # 【修改】
chunk_count = 0
...
try:
    for df_chunk in pd.read_csv(path_df_part_2, **csv_params):
        chunk_count += 1
        ...
        df_chunk_in_1 = df_chunk[df_chunk['time'] >= pd.to_datetime('2014-12-04')]  # 【不修改】Part2的最近1天
    ...
    f_U_part_2 = all_u_total.copy()
    f_U_part_2 = f_U_part_2.join(all_u_b4, how='left').fillna(0)
    f_U_part_2 = f_U_part_2.join(all_u_b3, how='left').fillna(0)
    f_U_part_2 = f_U_part_2.join(all_u_in_1, how='left').fillna(0)

    # 向量化计算购买转化率 【不修改 - 算法逻辑保持不变】
    f_U_part_2['u_b4_rate'] = f_U_part_2['u_b4_count_in_6'] / (f_U_part_2['u_b_total_in_6'] + 1e-10)
    ...
    if not all_u_b4_first.empty and not all_u_first.empty:
        ...
        f_U_part_2 = f_U_part_2.join(time_diff[['u_b4_diff_hours']], how='left').fillna(0)
    else:
        f_U_part_2['u_b4_diff_hours'] = 0

    # 最终用户特征 (5个) - 指定数据类型节省空间 【不修改 - 算法逻辑保持不变】
    f_U_part_2 = f_U_part_2[['u_b4_count_in_6', 'u_b3_count_in_6', 'u_b4_rate', 'u_b_count_in_1', 'u_b4_diff_hours']].round(3)
    f_U_part_2 = f_U_part_2.astype({'u_b4_count_in_6': 'int16', 'u_b3_count_in_6': 'int16', 'u_b_count_in_1': 'int16'})
    f_U_part_2.reset_index(inplace=True)

    # 性能优化：批量写入减少I/O
    f_U_part_2.to_csv(path_df_part_2_U, index=False)
    print(f"   用户(U)特征保存完成: {len(f_U_part_2):,}个用户, 5个特征")
...
###########################################
'''Step 2.2 商品(I)特征 - 精简版(5个特征) - DSW 48核心优化 【修改注释】
...
try:
    ...
    f_I_part_2 = all_i_u.join(all_i_b4, how='left').fillna(0)
    f_I_part_2 = f_I_part_2.join(all_i_b3, how='left').fillna(0)
    f_I_part_2 = f_I_part_2.join(all_i_total, how='left').fillna(0)
    f_I_part_2 = f_I_part_2.join(all_i_in_1, how='left').fillna(0)

    # 向量化计算转化率 【不修改 - 算法逻辑保持不变】
    f_I_part_2['i_b4_rate'] = f_I_part_2['i_b4_count_in_6'] / (f_I_part_2['i_b_total_in_6'] + 1e-10)

    # 最终商品特征 (5个) 【不修改 - 算法逻辑保持不变】
    f_I_part_2 = f_I_part_2[['i_u_count_in_6', 'i_b4_count_in_6', 'i_b3_count_in_6', 'i_b4_rate', 'i_b_count_in_1']].round(3)
    f_I_part_2 = f_I_part_2.astype({'i_u_count_in_6': 'int16', 'i_b4_count_in_6': 'int16', 'i_b3_count_in_6': 'int16', 'i_b_count_in_1': 'int16'})
    f_I_part_2.reset_index(inplace=True)

    f_I_part_2.to_csv(path_df_part_2_I, index=False)
    print(f"   商品(I)特征保存完成: {len(f_I_part_2):,}个商品, 5个特征")
...
###########################################
'''Step 2.3 品类(C)特征 - 精简版(5个特征) - DSW 48核心优化 【修改注释】'''

print("\nStep 2.3: 构建品类(C)特征 (5个核心特征)...")
...
try:
    ...
    f_C_part_2 = all_c_u.join(all_c_b4, how='left').fillna(0)
    f_C_part_2 = f_C_part_2.join(all_c_b3, how='left').fillna(0)
    f_C_part_2 = f_C_part_2.join(all_c_total, how='left').fillna(0)
    f_C_part_2 = f_C_part_2.join(all_c_in_1, how='left').fillna(0)

    f_C_part_2['c_b4_rate'] = f_C_part_2['c_b4_count_in_6'] / (f_C_part_2['c_b_total_in_6'] + 1e-10)

    f_C_part_2 = f_C_part_2[['c_u_count_in_6', 'c_b4_count_in_6', 'c_b3_count_in_6', 'c_b4_rate', 'c_b_count_in_1']].round(3)
    f_C_part_2 = f_C_part_2.astype({'c_u_count_in_6': 'int16', 'c_b4_count_in_6': 'int16', 'c_b3_count_in_6': 'int16', 'c_b_count_in_1': 'int16'})
    f_C_part_2.reset_index(inplace=True)

    f_C_part_2.to_csv(path_df_part_2_C, index=False)
    print(f"   品类(C)特征保存完成: {len(f_C_part_2):,}个品类, 5个特征")
...
###########################################
'''Step 2.4 商品-品类(IC)特征 - 精简版(5个特征) 【不修改】'''

print("\nStep 2.4: 构建商品-品类(IC)特征 (5个核心特征)...")

# 获取UIC数据 【不修改 - 算法逻辑保持不变】
try:
    df_uic = pd.read_csv(path_df_part_2_uic_label, dtype={'user_id': 'int32', 'item_id': 'int32', 'item_category': 'int16'})
    ...
    pd.DataFrame(columns=['item_id', 'item_category', 'ic_u_rank_in_c', 'ic_b4_rank_in_c', 'ic_b_rank_in_c']).to_csv(path_df_part_2_IC, index=False)
    df_uic = None

if df_uic is not None:
    # 获取商品特征用于排名 【不修改 - 算法逻辑保持不变】
    f_I_for_rank = pd.read_csv(path_df_part_2_I, dtype={'item_id': 'int32'})[['item_id', 'i_u_count_in_6', 'i_b4_count_in_6']]
    f_I_for_rank['i_b_count_in_6'] = f_I_for_rank['i_u_count_in_6'] + f_I_for_rank['i_b4_count_in_6']  # 简化的总行为数
    
    # 合并商品和品类信息 【不修改 - 算法逻辑保持不变】
    f_IC_part_2 = df_uic.merge(f_I_for_rank, on='item_id', how='left').fillna(0)
    
    # 性能优化：使用向量化rank操作 【不修改 - 算法逻辑保持不变】
    f_IC_part_2['ic_u_rank_in_c'] = f_IC_part_2.groupby('item_category')['i_u_count_in_6'].rank(method='min', ascending=False).astype('int16')
    f_IC_part_2['ic_b4_rank_in_c'] = f_IC_part_2.groupby('item_category')['i_b4_count_in_6'].rank(method='min', ascending=False).astype('int16')
    f_IC_part_2['ic_b_rank_in_c'] = f_IC_part_2.groupby('item_category')['i_b_count_in_6'].rank(method='min', ascending=False).astype('int16')
    
    # 最终IC特征 (5个) 【不修改 - 算法逻辑保持不变】
    f_IC_part_2 = f_IC_part_2[['item_id', 'item_category', 'ic_u_rank_in_c', 'ic_b4_rank_in_c', 'ic_b_rank_in_c']]
    f_IC_part_2 = f_IC_part_2.astype({'item_id': 'int32', 'item_category': 'int16'})
    f_IC_part_2.to_csv(path_df_part_2_IC, index=False)
    print(f"   商品-品类(IC)特征保存完成: {len(f_IC_part_2):,}个商品-品类对, 5个特征")

###########################################
'''Step 2.5 用户-商品(UI)特征 - 精简版(5个特征) - DSW 48核心优化 【修改注释】'''

print("\nStep 2.5: 构建用户-商品(UI)特征 (5个核心特征)...")
...
try:
    ...
    f_UI_part_2 = all_ui_total.join(all_ui_b4, how='left').fillna(0)
    f_UI_part_2 = f_UI_part_2.join(all_ui_b3, how='left').fillna(0)
    f_UI_part_2 = f_UI_part_2.join(all_ui_in_1, how='left').fillna(0)

    # 向量化排名计算 【不修改 - 算法逻辑保持不变】
    f_UI_part_2['ui_b_count_rank_in_u'] = f_UI_part_2.groupby('user_id')['ui_b_total_in_6'].rank(method='min', ascending=False).astype('int16')

    # 时间差计算 (Part2目标: 2014-12-05) 【不修改 - 算法逻辑保持不变】
    if not all_ui_b3_last.empty:
        target_time = pd.to_datetime('2014-12-05')  # 【不修改】Part2预测日期
        all_ui_b3_last['ui_b3_last_hours'] = (target_time - all_ui_b3_last['ui_b3_last_time']).dt.total_seconds() / 3600
        f_UI_part_2 = f_UI_part_2.join(all_ui_b3_last[['ui_b3_last_hours']], how='left').fillna(999999)
    else:
        f_UI_part_2['ui_b3_last_hours'] = 999999

    # 最终UI特征 (5个) 【不修改 - 算法逻辑保持不变】
    f_UI_part_2 = f_UI_part_2[['ui_b4_count_in_6', 'ui_b3_count_in_6', 'ui_b_count_rank_in_u', 'ui_b3_last_hours', 'ui_b_count_in_1']].round(1)
    f_UI_part_2 = f_UI_part_2.astype({'ui_b4_count_in_6': 'int16', 'ui_b3_count_in_6': 'int16', 'ui_b_count_in_1': 'int16'})
    f_UI_part_2.reset_index(inplace=True)

    f_UI_part_2.to_csv(path_df_part_2_UI, index=False)
    print(f"   用户-商品(UI)特征保存完成: {len(f_UI_part_2):,}个用户-商品对, 5个特征")
...
###########################################
'''Step 2.6 用户-品类(UC)特征 - 精简版(5个特征) - DSW 48核心优化 【修改注释】'''

print("\nStep 2.6: 构建用户-品类(UC)特征 (5个核心特征)...")
...
try:
    ...
    f_UC_part_2 = all_uc_total.join(all_uc_b4, how='left').fillna(0)
    f_UC_part_2 = f_UC_part_2.join(all_uc_b3, how='left').fillna(0)
    f_UC_part_2 = f_UC_part_2.join(all_uc_in_1, how='left').fillna(0)

    # 向量化排名计算 【不修改 - 算法逻辑保持不变】
    f_UC_part_2['uc_b_count_rank_in_u'] = f_UC_part_2.groupby('user_id')['uc_b_total_in_6'].rank(method='min', ascending=False).astype('int16')
    ...
    if not all_uc_b3_last.empty:
        ...
        f_UC_part_2 = f_UC_part_2.join(all_uc_b3_last[['uc_b3_last_hours']], how='left').fillna(999999)
    else:
        f_UC_part_2['uc_b3_last_hours'] = 999999

    # 最终UC特征 (5个) 【不修改 - 算法逻辑保持不变】
    f_UC_part_2 = f_UC_part_2[['uc_b4_count_in_6', 'uc_b3_count_in_6', 'uc_b_count_rank_in_u', 'uc_b3_last_hours', 'uc_b_count_in_1']].round(1)
    f_UC_part_2 = f_UC_part_2.astype({'uc_b4_count_in_6': 'int16', 'uc_b3_count_in_6': 'int16', 'uc_b_count_in_1': 'int16'})
    f_UC_part_2.reset_index(inplace=True)

    f_UC_part_2.to_csv(path_df_part_2_UC, index=False)
    print(f"   用户-品类(UC)特征保存完成: {len(f_UC_part_2):,}个用户-品类对, 5个特征")
...
print("\n" + "=" * 60)
print("Part 2精简特征工程完成! (DSW环境优化)") # 【修改】

# 最终统计 【不修改】
print(f"\n生成的特征文件:")
feature_files = [path_df_part_2_U, path_df_part_2_I, path_df_part_2_C, path_df_part_2_IC, path_df_part_2_UI, path_df_part_2_UC]
total_size = 0
